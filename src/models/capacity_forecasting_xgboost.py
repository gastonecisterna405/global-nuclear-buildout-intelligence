...
def _build_target(df: pd.DataFrame) -> pd.Series:
    # Scenario-derived heuristic: weights reflect typical pipeline conversion rates.
    # Not derived from historical snapshots — replace with real outcomes for production use.
    return (
        df["operating_capacity_mwe"]
        + 0.75 * df["construction_capacity_mwe"]
        + 0.45 * df["planned_capacity_mwe"]
        + 0.15 * df["proposed_capacity_mwe"]
    )


def train_capacity_forecaster(countries: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    df = countries.copy()
    df["target_capacity_2035_mwe"] = _build_target(df)
    ...
    base_model = (
        XGBRegressor(n_estimators=80, max_depth=3, learning_rate=0.08,
    ...
    metrics = {
        ...
        "notes": (
            "Target is a scenario-derived linear combination of pipeline features — not an "
            "independent ground truth. Metrics measure fit to the heuristic, not real-world accuracy. "
            "Replace with historical capacity snapshots for production use."
        ),
    }

    df["forecast_capacity_2035_mwe"] = pipe.predict(X).round(1)
    out = df[["country", "region", "forecast_capacity_2035_mwe",
              "target_capacity_2035_mwe", "policy_signal_score"]]

    out.to_csv(config.PREDICTIONS / "capacity_forecasts.csv", index=False)
    (config.METRICS / "forecasting_metrics.json").write_text(
